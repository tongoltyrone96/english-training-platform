...
export async function POST(request: Request) {
  const user = await getActiveUser(); if (!user) return NextResponse.json({ error: "로그인이 필요합니다." }, { status: 401 });
  if (!(await consumeRateLimit("test-evaluation", user.id, 12, 60_000))) return NextResponse.json({ error: "평가 요청이 너무 많습니다. 잠시 후 다시 시도해 주세요." }, { status: 429 });
  ...
  if (audio.size < 200 || audio.size > 10 * 1024 * 1024 || !allowedAudioTypes.some((type) => audio.type.startsWith(type))) return NextResponse.json({ error: "지원하지 않거나 비어 있는 음성입니다." }, { status: 415 });
  const prior = await db.examAnswer.findUnique({ where: { idempotencyKey: fields.data.idempotencyKey }, include: { examAttempt: true } });
  if (prior) return prior.examAttempt.userId === user.id ? NextResponse.json({ completed: prior.examAttempt.status === "COMPLETED" }) : NextResponse.json({ error: "접근할 수 없습니다." }, { status: 403 });
  ...
  if (!attempt || fields.data.position !== currentPosition || attempt.questionOrder[currentPosition] !== fields.data.questionId) return NextResponse.json({ error: "현재 시험 문제와 일치하지 않습니다." }, { status: 409 });
  ...
  if (!question) return NextResponse.json({ error: "시험 문제를 찾을 수 없습니다." }, { status: 404 });
  ...
  try {
    ...
    if (isMock) { const scores = { meaning: 92, pronunciation: 92, grammar: 92, naturalness: 92, fluency: 92, completeness: 92 }; const finalScore = calculateFinalScore(scores); evaluation = { scores, finalScore, passed: finalScore >= Number(attempt.exam.passScore), transcript: "[개발용 mock transcript]", correctedAnswer: question.sentence.primaryAnswer, feedbackKo: "개발용 mock 평가입니다.", missingMeanings: [] as string[], detectedIssues: [] as string[], speechDetails: { words: [] }, promptVersion: "mock-v1", speechProviderVersion: "mock-v1", languageModelVersion: "mock-v1", processingMs: 0 }; }
    ...
    return NextResponse.json({ error: code.includes("RECOGNITION") || code.includes("NO_SPEECH") ? "음성을 인식하지 못했습니다. 다시 녹음해 주세요." : "평가 서비스가 응답하지 않았습니다. 다시 시도해 주세요." }, { status: 502 });
  }
