...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ month?: string; date?: string; notice?: string }> }) {
  ...
  const todaySession = await db.trainingSession.findUnique({ where: { userId_localDate: { userId: user.id, localDate: todayDate } }, select: { status: true, settingsSnapshot: true } });
  const activeSnapshot = todaySession?.status === "ACTIVE" ? todaySession.settingsSnapshot as { questionCount?: number; timeLimitSec?: number; passScore?: string } : null;
    ...
    <div className="figma-dashboard-grid">
      <div className="dashboard-main-column">
        <section className="dashboard-hero-card">
          <div><p className="eyebrow">TODAY&apos;S SESSION</p><h2>{weekday === "Sun" ? "Take time to recharge" : `Welcome back, ${user.name}`}</h2><p>{weekday === "Sun" ? "Review your progress and get ready for a new week." : activeSnapshot ? `${activeSnapshot.questionCount ?? settings.trainingQuestionCount} sentences · ${activeSnapshot.timeLimitSec ?? settings.trainingTimeLimitSec} seconds each · ${activeSnapshot.passScore ?? settings.trainingPassScore.toString()} pass score · Current session` : `${settings.trainingQuestionCount} sentences · ${settings.trainingTimeLimitSec} seconds each · ${settings.trainingPassScore.toString()} pass score`}</p>{activeSnapshot && <small>Changes made by an administrator apply when your next session is created.</small>}</div>
