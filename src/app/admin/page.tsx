...
export default async function AdminPage({ searchParams }: { searchParams: Promise<{ trainingDate?: string }> }) {
  ...
  return <main className="dashboard figma-dashboard admin-dashboard"><header className="admin-title"><div><p className="eyebrow">ADMIN CONSOLE</p><h1>Workspace settings</h1><p>Manage training content, team access and daily presentation settings.</p></div><div className="admin-title-mark">A</div></header><div className="admin-sections">
    ...
    <section className="panel admin-section"><header className="panel-heading"><div><p className="eyebrow">DOCX IMPORT</p><h2>Import sentences</h2></div><span className="status-badge">.docx · 5MB</span></header><DocxImporter />{imports.length > 0 && <div className="import-history">{imports.map((item) => <span key={item.id}>{item.fileName} · {item.status} · {item.importedCount} sentences</span>)}</div>}</section>
    <section className="panel admin-section"><header className="panel-heading"><div><p className="eyebrow">TRAINING RULES</p><h2>Global settings</h2></div></header><SettingsForm settings={{ ...settings, trainingPassScore: settings.trainingPassScore.toString() }} /></section>
    <section className="panel admin-section"><header className="panel-heading"><div><p className="eyebrow">TEAM ACCESS</p><h2>Users and invitation codes</h2></div></header><InvitationForm /><div className="invitation-list">{invitations.map((invitation) => <form action={toggleInvitationAction} key={invitation.id}><span><strong>{invitation.label}</strong><small>{invitation.uses}/{invitation.maxUses} uses · {invitation.active ? "Active" : "Inactive"}</small></span><input type="hidden" name="id" value={invitation.id} /><button className="ghost-button">{invitation.active ? "Disable" : "Enable"}</button></form>)}</div><div className="user-list">{users.map((member) => <form action={updateUserAction} key={member.id}><span><strong>{member.name}{member.id === admin.id ? " (current user)" : ""}</strong><small>{member.email}</small></span><input type="hidden" name="id" value={member.id} /><select name="role" defaultValue={member.role} disabled={member.id === admin.id}><option value="USER">USER</option><option value="ADMIN">ADMIN</option></select><select name="status" defaultValue={member.status} disabled={member.id === admin.id}><option value="ACTIVE">ACTIVE</option><option value="INACTIVE">INACTIVE</option></select><button className="ghost-button" disabled={member.id === admin.id}>Save</button></form>)}</div></section>
    <section className="panel admin-section"><header className="panel-heading"><div><p className="eyebrow">AUDIT</p><h2>Recent admin activity</h2></div></header><div className="audit-list">{logs.length === 0 ? <p className="muted">No records yet.</p> : logs.map((log) => <div key={log.id}><strong>{log.action}</strong><span>{log.actor.name} · {log.createdAt.toLocaleString("ko-KR")}</span></div>)}</div></section></div>
  </main>;
}
